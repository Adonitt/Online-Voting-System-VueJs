...
const submitVote = async () => {
  ...
  await withLoading(async () => {
    ...
    if (selectedCandidates.value.length === 0 || selectedCandidates.value.length > 5) {
    ...
    try {
      ...
      selectedCandidates.value = [];
      hasVoted.value = true
      await router.push({name: 'my-vote-details', params: {id: store.loggedInUser?.id}})
    } catch (error) {
      if (hasVoted === true) {
      ...
      toast.showError(error.response?.data?.message || "There was an error casting your vote!");
    }
  });
};
    ...
    <div
      ...
      <div class="flex items-center justify-center mb-4">
        <svg class="w-6 h-6 text-green-600 mr-2" fill="none" stroke="currentColor" stroke-width="2"
             viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" style="height: 40px">
      ...
      <div class="col-md-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          <div
              ...
              :class="['card', selectedParty === party.id ? 'border border-primary shadow' : 'border']"
          ...
          >
            <div class="card-body p-4 flex flex-col items-center text-center">
              ...
              <span class="text-lg font-semibold">Party Number: <b>{{ party.id }}</b></span>
            </div>
          </div>
        </div>
      </div>
                  ...
                  <div class="">
                    <div class="media d-flex align-items-center">
                      ...
                      <div class="media-body">
                        <p class="mb-1">{{ candidate.firstName }} {{ candidate.lastName }}</p>
                      </div>
                    </div>
                  </div>
                </label>
